...
public class UITabuleiro : MonoBehaviour
{
	...
	public UIPiece[,] pieces = new UIPiece[8, 8];
	...
	private void Update ()
	{
		...
		//if
		{
			...
			if (Input.GetMouseButtonDown(0))
			{
				...
				{
					SelectUIPiece(x, y); //função que serve para verificar que a peça foi selecionada
				}
			}
	...
	{ //função responsável por lidar com os movimentos do mouse
	...
	private void SelectUIPiece (int x, int y)
	{
		//fora da matriz
		...
		{ //checa os limites da matriz na interface do tabuleiro
			Debug.Log(x);
			Debug.Log(y);
			return;
		}
		...
		UIPiece p = pieces[x / 2, y / 2 + 11]; //procura a peça no x,y de quando o clique começou
		if (p != null)
		{
			selectedUIPiece = p;
			startDrag = mouseOver;
			Debug.Log(selectedUIPiece.name);
		}
	}

	private void TryMove (int x1, int y1, int x2, int y2)
	{
		selectedUIPiece = pieces[x1 / 2, 7 - (y1 / 2 + 11)]; //procura a peça nas posições de quando o clique começou
		...
		MoveUIPiece(selectedUIPiece, x2, 7 - y2); //move a peça para o clique terminou
	}
	...
	private void GenerateBoard (bool inverted = false)
	{ //tabuleiro gerado para caso a cor escolhida seja preto quando inverted é false
		...
		{ //primeiras duas linhas do tabuleiro
			...
			{ //cada coluna do tabuleiro
				if (inverted) GenerateBlackUIPiece(x, y);
				else GenerateWhiteUIPiece(x, y);
			}
		}
		for (int y = 6; y < 8; y++)
		{ //últimas duas linhas do tabuleiro
			for (int x = 0; x < 8; x++)
			{ //cada coluna do tabuleiro
				if (inverted) GenerateWhiteUIPiece(x, y);
				else GenerateBlackUIPiece(x, y);
			}
		}
	}

	private void GenerateBoardInverted ()
	{ //tabuleiro gerado para caso a cor escolhida seja bege
		GenerateBoard(inverted:true);
	}

	private void GenerateWhiteUIPiece (int x, int y)
	{//função responsável por gerar as peças brancas na matriz do tabuleiro
		GameObject go = Instantiate(whiteStonePawnPrefab) as GameObject;
		go.transform.SetParent(transform);
		UIPiece p = go.GetComponent<UIPiece>();
		pieces[x, y] = p;
		MoveUIPiece(p, x, y);
	}

	private void GenerateBlackUIPiece (int x, int y)
	{ //função responsável por gerar as peças pretas na matriz do tabuleiro
